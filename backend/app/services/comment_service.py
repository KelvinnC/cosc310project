...
def create_comment(payload: CommentCreate, user_id: str) -> Comment:
    comments = load_all()
    new_comment_id = max((com.get("id", 0) for com in comments), default=0) + 1

    new_comment = Comment(id=new_comment_id, reviewId=payload.reviewId, authorId=user_id, commentBody=payload.commentBody, date=datetime.datetime.now())
